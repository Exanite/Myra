	...
	public class Project
	{
		...
		internal static LoadContext CreateLoadContext(IAssetManager assetManager)
		{
			Func<Type, string, object> resourceGetter = (t, name) =>
			{
				...
				{
					return assetManager.GetAsset<TextureRegion>(name);
				}
				else if (t == typeof(SpriteFontBase))
				{
					return assetManager.GetAsset<SpriteFontBase>(name);
				}
		...
		public static Project LoadFromXml<T>(XDocument xDoc, IAssetManager assetManager = null, T handler = null) where T : class
		{
			...
			{
				stylesheet = assetManager.GetAsset<Stylesheet>(stylesheetPathAttr.Value);
			}
		...
		public static object LoadObjectFromXml<T>(string data, IAssetManager assetManager = null, Stylesheet stylesheet = null, T handler = null, Type parentType = null) where T : class
		{
